...
function authorized(request: NextRequest) {
  return hasAdminSession() && isSameOrigin(request);
}

export async function GET() {
  if (!hasAdminSession()) return jsonError("unauthorized", 401);
  const db = getSupabaseAdmin() as any;
  const [{ data: partners }, { data: payments }, { data: requests }, { data: quotas }, { data: notes }] = await Promise.all([
  ...
  return NextResponse.json({ partners: partners || [], payments: payments || [], requests: requests || [], quotas: quotas || [], notes: notes || [] });
}
  ...
  try {
    ...
    if (input.action === "partner_status" && partnerStatuses.includes(input.status)) {
      ...
      if (input.status === "bezahlt" && payment) {
        await db.from("partners").update({ payment_status: "bezahlt", status: "aktiv" }).eq("id", payment.partner_id);
        if (payment.package_name === "Keyword Support Paket") await db.from("partner_monthly_quotas").upsert({ partner_id: payment.partner_id, month_key: monthKey(), package_name: payment.package_name, quota_total: 4, quota_used: 0, quota_remaining: 4, payment_required: true, is_active: true }, { onConflict: "partner_id,month_key" });
      ...
      await db.from("partner_monthly_quotas").update({ quota_total: Number(input.total), quota_remaining: Number(input.remaining), is_active: Boolean(input.is_active) }).eq("id", input.quota_id);
      ...
      const { data: answered } = await db.from("partner_keyword_requests").update({ status: input.status, admin_answer: input.admin_answer || null, recommended_keywords: keywords, search_intent_notes: input.search_intent_notes || null, optimization_notes: input.optimization_notes || null, answered_at: input.status === "beantwortet" ? new Date().toISOString() : null }).eq("id", input.request_id).select("partner_id").single();
      if (input.status === "beantwortet" && answered) {
        const { data: partner } = await db.from("partners").select("contact_name,email").eq("id", answered.partner_id).single();
        if (partner) mailSent = (await sendPartnerEmail(partner.email, partnerEmailTemplates.answered(partner.contact_name))).sent;
      }
    } else if (input.action === "note" && input.note) {
      await db.from("partner_admin_notes").insert({ partner_id: input.partner_id, note: String(input.note).trim() });
    } else {
      return jsonError("invalid", 400);
    }
    return NextResponse.json({ ok: true, mailSent });
  } catch (error) {
    console.error("Partner admin update failed", { name: error instanceof Error ? error.name : "UnknownError" });
